...
param(
    ...
    [string]$Zip = "$HOME\Downloads\atualizacao-81-usabilidade-entrada-ferramentas-exclusao-material.zip"
)
...
if (!(Test-Path -LiteralPath (Join-Path $Repo '.git') -PathType Container)) {
    throw "A pasta informada não é o repositório Git esperado: $Repo"
}
...
$targets = @(
    ...
    "backend/app/routes/materialRoutes.js",
    "backend/scripts/test-update-81-stock-usability.js",
    ...
    "frontend/src/utils/numberInputWheelGuard.js"
)
...
if ($existingChanges.Count -gt 0) {
    Write-Host "Existem alterações locais nos arquivos usados pela Atualização 81:" -ForegroundColor Yellow
...
$temp = Join-Path $env:TEMP "estoque-superinfra-atualizacao-81-$stamp"
$backup = Join-Path $HOME "Documents\backup-estoque-superinfra-atualizacao-81-$stamp"
...
try {
    Write-Host "Extraindo Atualização 81..." -ForegroundColor Cyan
    ...
    Write-Host "Testando filtros, scroll, ferramentas e exclusão..." -ForegroundColor Cyan

    Invoke-NativeChecked `
        -Command "node" `
        -Arguments @("backend/scripts/test-update-81-stock-usability.js") `
        -Description "Teste da Atualização 81"
    ...
    Write-Host ""
    Write-Host "Atualização 81 aplicada e validada com sucesso." -ForegroundColor Green
    Write-Host "Backup local: $backup" -ForegroundColor Green
    Write-Host "Nenhuma migration, exclusão automática ou movimentação de saldo foi executada." -ForegroundColor Green
    Write-Host ""
    git status --short
}
catch {
    ...
    if (Test-Path -LiteralPath $newFilesManifest) {
        Get-Content -LiteralPath $newFilesManifest |
            ForEach-Object {
