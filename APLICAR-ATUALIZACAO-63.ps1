...
param(
    ...
    [string]$Zip = "$HOME\Downloads\atualizacao-63-cidade-solicitacao-material.zip"
)
...
$existingTargetChanges = @(
    git status --porcelain -- $sourceTargets
)
...
if ($existingTargetChanges.Count -gt 0) {
    Write-Host "Existem alterações locais nos arquivos que esta atualização precisa substituir:" -ForegroundColor Yellow
    $existingTargetChanges | ForEach-Object { Write-Host $_ -ForegroundColor Yellow }
    throw "Faça commit ou backup dessas alterações antes de aplicar a atualização 63."
}

$stamp = Get-Date -Format 'yyyyMMdd-HHmmss'
$temp = Join-Path $env:TEMP "estoque-superinfra-atualizacao-63-$stamp"
$backup = Join-Path $HOME "Documents\backup-estoque-superinfra-atualizacao-63-$stamp"
...
try {
    Write-Host "Extraindo atualização 63..." -ForegroundColor Cyan
    ...
    if (!(Test-Path -LiteralPath (Join-Path $Repo "frontend\node_modules"))) {
        Write-Host "Instalando dependências do frontend para validação local..." -ForegroundColor Cyan
    ...
    Write-Host ""
    Write-Host "Atualização 63 aplicada e validada com sucesso." -ForegroundColor Green
    Write-Host "Backup local: $backup" -ForegroundColor Green
    Write-Host "Nenhuma alteração foi feita no banco de dados." -ForegroundColor Green
    Write-Host ""
    git status --short
}
catch {
