export const MODULES = [
  ...
  { key: 'technicianTools', label: 'Ferramentas na ficha do técnico', group: 'Cadastros e estoque', roles: ['admin', 'supervisor', 'estoquista'], routes: [] },
  { key: 'technicianEdit', label: 'Editar cadastro de técnicos', group: 'Permissões especiais', roles: ['admin', 'supervisor', 'estoquista'], routes: [] },
...
export const ADMIN_ONLY_MODULE_KEYS = ['users'];
export const ASSIGNABLE_MODULE_KEYS = ALL_MODULE_KEYS.filter((key) => !ADMIN_ONLY_MODULE_KEYS.includes(key));

export const DEFAULT_MODULES_BY_ROLE = {
  admin: ALL_MODULE_KEYS,
  supervisor: ALL_MODULE_KEYS.filter((key) => !['users', 'technicianEdit', 'technicianTransferLimitManage'].includes(key)),
  estoquista: [
    'operationsCockpit',
    'approvals',
    'materialRequests',
...
export function assignableModulesForRole(role) {
...
export function allowedModulesForRole(role) {
  return assignableModulesForRole(role);
}
