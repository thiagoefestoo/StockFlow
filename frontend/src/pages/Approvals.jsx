...
export default function Approvals() {
  ...
  function canApprove(item) {
    ...
    if (['warehouse_transfer', 'warehouse_delete'].includes(item.entityType)) return false;
    const limit = Number(user?.approvalLimit || 0);
    return limit > 0 && Number(item.amount || 0) <= limit;
  }

  async function decide() {
    if (!decision.item || deciding) return;
    setDeciding(true);
    try {
  ...
  return (
    <div className="page-grid erp-page">
          ...
          {approvals.map((a) => (
            <article className="approval-card" key={a.id}>
              ...
              <div className="approval-side">
                ...
                <div className="row-actions"><button className="info" onClick={() => setDetails(a)}>Detalhes</button>{a.status === 'pendente' && <><button disabled={!canApprove(a)} title={!canApprove(a) ? 'Valor acima do seu limite de aprovação ou operação reservada ao administrador.' : ''} onClick={() => setDecision({ open: true, type: 'approve', item: a, notes: '' })}>Aprovar</button><button className="ghost danger-outline" disabled={!canApprove(a)} title={!canApprove(a) ? 'Valor acima do seu limite de aprovação ou operação reservada ao administrador.' : ''} onClick={() => setDecision({ open: true, type: 'reject', item: a, notes: '' })}>Reprovar</button></>}</div>
              </div>
            </article>
          ))}
      ...
      <Modal open={decision.open} title={decision.type === 'approve' ? 'Aprovar item' : 'Reprovar item'} onClose={() => setDecision({ open: false, type: '', item: null, notes: '' })} footer={<><button className="ghost" disabled={deciding} onClick={() => setDecision({ open: false, type: '', item: null, notes: '' })}>Cancelar</button><button disabled={deciding || !canApprove(decision.item)} onClick={decide}>{deciding ? 'Registrando...' : 'Registrar decisão'}</button></>}>
        <p><strong>{decision.item?.title}</strong></p>
        {decision.item?.entityType === 'material_request' && (
          <div className="detail-grid compact">
            <div className="detail-card"><span>Valor solicitado</span><strong>{brl(decision.item?.amount)}</strong></div>
            <div className="detail-card"><span>Limite do técnico sem aprovação</span><strong>{brl(decision.item?.payload?.technicianApprovalLimit)}</strong></div>
            <div className="detail-card"><span>Seu limite de aprovação</span><strong>{isAdmin ? 'Sem limite' : brl(user?.approvalLimit)}</strong></div>
          </div>
        )}
        {!canApprove(decision.item) && <div className="alert danger">Você não possui permissão ou limite suficiente para registrar esta decisão.</div>}
