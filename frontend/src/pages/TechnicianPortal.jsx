...
export default function TechnicianPortal() {
  ...
  return (
    <div className="page-grid mobile-first">
      ...
      {isSupervisor && <section className={`panel technician-operator-panel ${selectedTech ? 'technician-selected' : 'technician-required'}`}><label>Selecionar técnico para simulação<select value={selectedTech} onChange={(e) => { setSelectedTech(e.target.value); loadStock(e.target.value); }}><option value="">Selecione</option>{technicians.map((t) => <option key={t.id} value={t.id}>{t.name}</option>)}</select></label></section>}
