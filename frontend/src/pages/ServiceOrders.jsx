  ...
  return (
    <div className="page-grid">
      ...
      <section className="panel">
        <div className="table-wrap"><table><thead><tr><th>OS</th><th>Cliente</th><th>Nº Contrato</th><th>Técnico</th><th>Tipo</th><th>Status</th><th>Materiais</th><th className="action-cell">Opções</th></tr></thead><tbody>{orders.map((o) => <tr key={o.id}><td>{o.osNumber}</td><td>{o.customerName}</td><td>{o.customerCpf}</td><td>{o.Technician?.name}</td><td>{serviceTypeLabel(o.serviceType)}</td><td>{o.status}</td><td>{o.ServiceOrderMaterials?.map((m) => m.serialNumber || `${m.Material?.name} (${formatQuantity(m.quantity)})`).join(', ')}</td><td><div className="action-toolbar"><button className="info" onClick={() => setDetails(o)}>Detalhes</button>{canReplaceEquipment && orderHasInstalledEquipment(o) && <button className="warning" onClick={() => openReplacement(o)}>Trocar equipamento</button>}{isAdmin && <button className="ghost" onClick={() => openEdit(o)}>Editar</button>}</div></td></tr>)}</tbody></table></div>
