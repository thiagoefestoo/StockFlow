  ...
  return (
    <div className="page-grid">
      ...
      <DetailsModal open={!!details} title={`Detalhes da OS ${details?.osNumber || ''}`} onClose={() => setDetails(null)} footer={<><button className="ghost" onClick={() => setDetails(null)}>Fechar</button>{isAdmin && details && <button onClick={() => { openEdit(details); setDetails(null); }}>Editar OS</button>}</>}>
        {details && <><DetailGrid fields={[["OS", details.osNumber], ["Cliente", details.customerName], ["Nº Contrato", details.customerCpf], ["Endereço", details.customerAddress], ["Cidade", details.city], ["Técnico", details.Technician?.name], ["Tipo", details.serviceType], ["Status", details.status], ["Concluída em", details.completedAt], ["Criada em", details.createdAt], ["Observações", details.notes]]} /><DetailList title="Materiais baixados" items={details.ServiceOrderMaterials || []} render={(item) => <><b>{item.Material?.name || 'Material'}</b><span>Qtd. {formatQuantity(item.quantity)} • {item.serialNumber || 'sem serial'} • {brl(item.totalCost)}</span></>} /><div className="viz-callout">Baixas por OS alimentam automaticamente caixa do técnico, estoque, patrimônio, histórico e BI.</div></>}
      </DetailsModal>
    </div>
  );
}
