...
export default function ServiceOrders() {
  ...
  return (
    <div className="page-grid">
      ...
      <section className="panel"><div className="table-wrap"><table><thead><tr><th>OS</th><th>Cliente</th><th>CPF</th><th>Técnico</th><th>Tipo</th><th>Status</th><th>Materiais</th><th className="action-cell">Opções</th></tr></thead><tbody>{orders.map((o) => <tr key={o.id}><td>{o.osNumber}</td><td>{o.customerName}</td><td>{o.customerCpf}</td><td>{o.Technician?.name}</td><td>{o.serviceType}</td><td>{o.status}</td><td>{o.ServiceOrderMaterials?.map((m) => m.serialNumber || `${m.Material?.name} (${m.quantity})`).join(', ')}</td><td><div className="action-toolbar"><button className="info" onClick={() => setDetails(o)}>Detalhes</button>{isAdmin && <button className="ghost" onClick={() => openEdit(o)}>Editar</button>}</div></td></tr>)}</tbody></table></div></section>
      ...
      <DetailsModal open={!!details} title={`Detalhes da OS ${details?.osNumber || ''}`} onClose={() => setDetails(null)} footer={<><button className="ghost" onClick={() => setDetails(null)}>Fechar</button>{isAdmin && details && <button onClick={() => { openEdit(details); setDetails(null); }}>Editar OS</button>}</>}>
        {details && <><DetailGrid fields={[["OS", details.osNumber], ["Cliente", details.customerName], ["CPF", details.customerCpf], ["Endereço", details.customerAddress], ["Cidade", details.city], ["Técnico", details.Technician?.name], ["Tipo", details.serviceType], ["Status", details.status], ["Concluída em", details.completedAt], ["Criada em", details.createdAt], ["Observações", details.notes]]} /><DetailList title="Materiais baixados" items={details.ServiceOrderMaterials || []} render={(item) => <><b>{item.Material?.name || 'Material'}</b><span>Qtd. {item.quantity} • {item.serialNumber || 'sem serial'} • {brl(item.totalCost)}</span></>} /><div className="viz-callout">Baixas por OS alimentam automaticamente caixa do técnico, estoque, patrimônio, histórico e BI.</div></>}
      </DetailsModal>
    </div>
  );
}
