...
export default function TechnicianInbox() {
  ...
  async function saveOs() {
    try {
      const payload = { ...osForm, technicianId: selectedTech, materials: osForm.materials.map((m) => { const selectedSerials = Array.isArray(m.serialNumbers) && m.serialNumbers.length ? m.serialNumbers : String(m.serialNumbersText || '').split(/\n|,|;/).map((s) => s.trim()).filter(Boolean); return { ...m, serialNumbers: selectedSerials, serialNumbersText: selectedSerials.join('\n'), quantity: selectedSerials.length || m.quantity }; }) };
