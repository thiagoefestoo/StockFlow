...
export default function TechnicianInbox() {
  ...
  function validateOs() {
    ...
    if (!String(osForm.city || '').trim()) return 'Informe a cidade onde o técnico está realizando a OS.';
      ...
      <section className="two-col technician-work-area">
        <article className={`panel os-work-card ${mobileSectionClass('baixa')}`}>
          ...
          <div className={`form-grid os-mobile-fields ${osFieldsOpen ? 'open' : ''}`}>
            ...
            <label>Cidade da OS<input value={osForm.city} maxLength={120} onChange={(e) => setOsForm({ ...osForm, city: e.target.value })} placeholder="Digite a cidade do atendimento" required /><small>{linkedWarehouseName ? `Preenchida inicialmente pela cidade do estoque ${linkedWarehouseName}. A baixa continua vinculada a esse estoque, mas a cidade pode ser alterada.` : 'Defina o estoque padrão no cadastro do técnico.'}</small></label>
