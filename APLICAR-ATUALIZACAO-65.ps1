...
param(
    ...
    [string]$Zip = "$HOME\Downloads\atualizacao-65-dicas-inteligentes-dinamicas.zip"
)
...
if ($existingChanges.Count -gt 0) {
    Write-Host "Existem alterações locais nos arquivos das dicas:" -ForegroundColor Yellow
    $existingChanges | ForEach-Object { Write-Host $_ -ForegroundColor Yellow }
    throw "Faça commit ou backup dessas alterações antes de aplicar a atualização 65."
}

$stamp = Get-Date -Format 'yyyyMMdd-HHmmss'
$temp = Join-Path $env:TEMP "estoque-superinfra-atualizacao-65-$stamp"
$backup = Join-Path $HOME "Documents\backup-estoque-superinfra-atualizacao-65-$stamp"
...
try {
    Write-Host "Extraindo atualização 65..." -ForegroundColor Cyan
    ...
    Write-Host ""
    Write-Host "Atualização 65 aplicada e validada com sucesso." -ForegroundColor Green
