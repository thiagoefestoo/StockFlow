...
param(
    ...
    [string]$Zip = "$HOME\Downloads\atualizacao-74-inativar-usuario-limite-contas-ativas.zip"
)
...
if ($existingChanges.Count -gt 0) {
    Write-Host "Existem alterações locais nos arquivos usados pela atualização 74:" -ForegroundColor Yellow
...
$temp = Join-Path $env:TEMP "estoque-superinfra-atualizacao-74-$stamp"
$backup = Join-Path $HOME "Documents\backup-estoque-superinfra-atualizacao-74-$stamp"
...
try {
    Write-Host "Extraindo atualização 74..." -ForegroundColor Cyan
    ...
    Write-Host "Testando inativação e limite de contas ativas..." -ForegroundColor Cyan

    Invoke-NativeChecked `
        -Command "node" `
        -Arguments @("backend/scripts/test-user-inactivation-active-limit.js") `
        -Description "Teste de inativação de usuários"
    ...
    Write-Host ""
    Write-Host "Atualização 74 aplicada e validada com sucesso." -ForegroundColor Green
    Write-Host "Backup local: $backup" -ForegroundColor Green
    Write-Host "Nenhum usuário ou registro histórico foi alterado pela instalação." -ForegroundColor Green
    Write-Host ""
    git status --short
}
catch {
