...
param(
    ...
    [string]$Zip = "$HOME\Downloads\atualizacao-69-cadastrar-material-todos-estoques.zip"
)
...
if ($existingChanges.Count -gt 0) {
    Write-Host "Existem alterações locais nos arquivos usados pela atualização 69:" -ForegroundColor Yellow
...
$temp = Join-Path $env:TEMP "estoque-superinfra-atualizacao-69-$stamp"
$backup = Join-Path $HOME "Documents\backup-estoque-superinfra-atualizacao-69-$stamp"
...
try {
    Write-Host "Extraindo atualização 69..." -ForegroundColor Cyan
    ...
    Write-Host ""
    Write-Host "Atualização 69 aplicada e validada com sucesso." -ForegroundColor Green
    Write-Host "Backup local: $backup" -ForegroundColor Green
    Write-Host "Nenhuma migration ou alteração automática de materiais antigos." -ForegroundColor Green
    Write-Host ""
    git status --short
}
catch {
