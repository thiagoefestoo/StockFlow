...
param(
    ...
    [string]$Zip = "$HOME\Downloads\atualizacao-70-permissao-cadastro-todos-estoques.zip"
)
...
if ($existingChanges.Count -gt 0) {
    Write-Host "Existem alterações locais nos arquivos usados pela atualização 70:" -ForegroundColor Yellow
...
$temp = Join-Path $env:TEMP "estoque-superinfra-atualizacao-70-$stamp"
$backup = Join-Path $HOME "Documents\backup-estoque-superinfra-atualizacao-70-$stamp"
...
try {
    Write-Host "Extraindo atualização 70..." -ForegroundColor Cyan
    ...
    Write-Host "Testando a nova permissão..." -ForegroundColor Cyan

    Invoke-NativeChecked `
        -Command "node" `
        -Arguments @("backend/scripts/test-material-all-warehouses-permission.js") `
        -Description "Teste da permissão"
    ...
    Write-Host ""
    Write-Host "Atualização 70 aplicada e validada com sucesso." -ForegroundColor Green
    Write-Host "Backup local: $backup" -ForegroundColor Green
    Write-Host "Nenhuma migration ou alteração automática de usuários antigos." -ForegroundColor Green
    Write-Host ""
    git status --short
}
catch {
