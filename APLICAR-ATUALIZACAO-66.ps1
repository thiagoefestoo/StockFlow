...
param(
    ...
    [string]$Zip = "$HOME\Downloads\atualizacao-66-historico-retorno-tecnico.zip"
)
...
if ($existingChanges.Count -gt 0) {
    Write-Host "Existem alterações locais nos arquivos usados por esta atualização:" -ForegroundColor Yellow
    $existingChanges | ForEach-Object { Write-Host $_ -ForegroundColor Yellow }
    throw "Faça commit ou backup dessas alterações antes de aplicar a atualização 66."
}

$stamp = Get-Date -Format 'yyyyMMdd-HHmmss'
$temp = Join-Path $env:TEMP "estoque-superinfra-atualizacao-66-$stamp"
$backup = Join-Path $HOME "Documents\backup-estoque-superinfra-atualizacao-66-$stamp"
...
try {
    Write-Host "Extraindo atualização 66..." -ForegroundColor Cyan
    ...
    Write-Host ""
    Write-Host "Atualização 66 aplicada e validada com sucesso." -ForegroundColor Green
