...
param(
    ...
    [string]$Zip = "$HOME\Downloads\atualizacao-64-correcao-bi-financeiro.zip"
)
...
if ($existingChanges.Count -gt 0) {
    Write-Host "Existem alterações locais nos arquivos do BI Financeiro:" -ForegroundColor Yellow
    $existingChanges | ForEach-Object { Write-Host $_ -ForegroundColor Yellow }
    throw "Faça commit ou backup dessas alterações antes de aplicar a atualização 64."
}

$stamp = Get-Date -Format 'yyyyMMdd-HHmmss'
$temp = Join-Path $env:TEMP "estoque-superinfra-atualizacao-64-$stamp"
$backup = Join-Path $HOME "Documents\backup-estoque-superinfra-atualizacao-64-$stamp"
...
try {
    Write-Host "Extraindo atualização 64..." -ForegroundColor Cyan
    ...
    Write-Host "Testando as fórmulas do BI Financeiro..." -ForegroundColor Cyan

    Invoke-NativeChecked `
        -Command "node" `
        -Arguments @("backend/scripts/test-financial-bi.js") `
        -Description "Teste do BI Financeiro"

    if (!(Test-Path -LiteralPath (Join-Path $Repo "frontend\node_modules"))) {
        Write-Host "Instalando dependências do frontend para a validação..." -ForegroundColor Cyan
    ...
    Write-Host ""
    Write-Host "Atualização 64 aplicada e validada com sucesso." -ForegroundColor Green
    Write-Host "Backup local: $backup" -ForegroundColor Green
    Write-Host "Banco de dados: nenhuma migration ou alteração de registros." -ForegroundColor Green
    Write-Host ""
    git status --short
}
catch {
    ...
    $backupFiles = @(
        Get-ChildItem -LiteralPath $backup -File -Recurse |
            Where-Object { $_.FullName -ne $newFilesManifest }
    )
