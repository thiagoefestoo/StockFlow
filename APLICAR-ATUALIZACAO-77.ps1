...
param(
    ...
    [string]$Zip = "$HOME\Downloads\atualizacao-77-revisoes-antes-confirmacao.zip"
)
...
if ($existingChanges.Count -gt 0) {
    Write-Host "Existem alterações locais nos arquivos usados pela atualização 77:" -ForegroundColor Yellow
...
$temp = Join-Path $env:TEMP "estoque-superinfra-atualizacao-77-$stamp"
$backup = Join-Path $HOME "Documents\backup-estoque-superinfra-atualizacao-77-$stamp"
...
try {
    Write-Host "Extraindo atualização 77..." -ForegroundColor Cyan
    ...
    Write-Host "Testando todos os menus de revisão..." -ForegroundColor Cyan

    Invoke-NativeChecked `
        -Command "node" `
        -Arguments @("frontend/scripts/test-operation-review-flows.js") `
        -Description "Teste dos menus de revisão"
    ...
    Write-Host ""
    Write-Host "Atualização 77 aplicada e validada com sucesso." -ForegroundColor Green
    Write-Host "Backup local: $backup" -ForegroundColor Green
    Write-Host "Nenhuma migration, alteração de backend ou movimentação de saldo." -ForegroundColor Green
    Write-Host ""
    git status --short
}
catch {
