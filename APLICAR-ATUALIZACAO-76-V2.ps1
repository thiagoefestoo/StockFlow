...
param(
    ...
    [string]$Zip = "$HOME\Downloads\atualizacao-76-v2-multiplas-ferramentas-por-tecnico.zip"
)
...
function Assert-FileContains {
    ...
    foreach ($marker in $Markers) {
        if (!$content.Contains($marker)) {
            throw "Validação não encontrou o marcador '$marker' em $Path."
        }
    }
}
...
if ($existingChanges.Count -gt 0) {
    Write-Host "Existem alterações locais na página de Técnicos:" -ForegroundColor Yellow
    $existingChanges | ForEach-Object { Write-Host $_ -ForegroundColor Yellow }
    throw "Faça commit ou backup dessas alterações antes de aplicar a atualização 76 V2."
}

$stamp = Get-Date -Format 'yyyyMMdd-HHmmss'
$temp = Join-Path $env:TEMP "estoque-superinfra-atualizacao-76-v2-$stamp"
$backup = Join-Path $HOME "Documents\backup-estoque-superinfra-atualizacao-76-v2-$stamp"
...
try {
    Write-Host "Extraindo atualização 76 V2..." -ForegroundColor Cyan
    ...
    Write-Host "Validando backend existente..." -ForegroundColor Cyan
    ...
    Write-Host "Validando integração de vários itens em UTF-8..." -ForegroundColor Cyan

    Assert-FileContains `
        -Path (Join-Path $Repo "frontend\src\pages\Technicians.jsx") `
        -Markers @(
            "Adicionar outra ferramenta",
            "toolForm.items.map",
            "OperationReviewModal",
            "items,",
            "selectedIds.has(materialId)"
        )

    Assert-FileContains `
        -Path (Join-Path $Repo "backend\app\controllers\technicianToolController.js") `
        -Markers @(
    ...
    Write-Host ""
    Write-Host "Atualização 76 V2 aplicada e validada com sucesso." -ForegroundColor Green
