...
param(
    ...
    [string]$Zip = "$HOME\Downloads\atualizacao-79-corrige-transferencia-entre-estoques.zip"
)
...
if ($existingChanges.Count -gt 0) {
    Write-Host "Existem alterações locais nos arquivos usados pela atualização 79:" -ForegroundColor Yellow
...
$temp = Join-Path $env:TEMP "estoque-superinfra-atualizacao-79-$stamp"
$backup = Join-Path $HOME "Documents\backup-estoque-superinfra-atualizacao-79-$stamp"
...
try {
    Write-Host "Extraindo atualização 79..." -ForegroundColor Cyan
    ...
    Write-Host "Testando a transferência entre estoques..." -ForegroundColor Cyan

    Invoke-NativeChecked `
        -Command "node" `
        -Arguments @("backend/scripts/test-warehouse-transfer-flow.js") `
        -Description "Teste da transferência entre estoques"
    ...
    Write-Host ""
    Write-Host "Atualização 79 aplicada e validada com sucesso." -ForegroundColor Green
    Write-Host "Backup local: $backup" -ForegroundColor Green
    Write-Host "Nenhuma migration e nenhum saldo foram alterados pela instalação." -ForegroundColor Green
    Write-Host ""
    git status --short
}
catch {
