...
param(
    ...
    [string]$Zip = "$HOME\Downloads\atualizacao-62-v2-limite-conector-verde-os.zip"
)
...
if (!(Test-Path -LiteralPath (Join-Path $Repo '.git') -PathType Container)) {
    throw "A pasta informada não parece ser o repositório Git: $Repo"
}

$stamp = Get-Date -Format 'yyyyMMdd-HHmmss'
$temp = Join-Path $env:TEMP "estoque-superinfra-atualizacao-62-v2-$stamp"
$backup = Join-Path $HOME "Documents\backup-estoque-superinfra-atualizacao-62-v2-$stamp"
...
try {
    Write-Host "Extraindo atualização 62 V2..." -ForegroundColor Cyan
    ...
    if (Test-Path -LiteralPath (Join-Path $Repo 'frontend\build')) {
        ...
        Copy-Item -LiteralPath (Join-Path $Repo 'frontend\build') -Destination $backupBuild -Recurse -Force
    }
    ...
    if (!(Test-Path -LiteralPath (Join-Path $Repo 'backend\node_modules'))) {
        Invoke-NativeChecked -Command 'npm' -Arguments @('install', '--prefix', 'backend') -Description 'Instalação do backend'
    }

    if (!(Test-Path -LiteralPath (Join-Path $Repo 'frontend\node_modules'))) {
        Invoke-NativeChecked -Command 'npm' -Arguments @('install', '--prefix', 'frontend') -Description 'Instalação do frontend'
    }

    Write-Host "Validando backend..." -ForegroundColor Cyan
    Invoke-NativeChecked -Command 'npm' -Arguments @('run', 'check', '--prefix', 'backend') -Description 'Validação do backend'

    Write-Host "Testando a regra de limite por OS..." -ForegroundColor Cyan
    Invoke-NativeChecked -Command 'node' -Arguments @('backend/scripts/test-service-order-quantity-limit.js') -Description 'Teste do limite por OS'

    Write-Host "Compilando frontend..." -ForegroundColor Cyan
    Invoke-NativeChecked -Command 'npm' -Arguments @('run', 'build', '--prefix', 'frontend') -Description 'Build do frontend'
    ...
    Invoke-NativeChecked -Command 'git' -Arguments (@('diff', '--check', '--') + $gitCheckPaths) -Description 'git diff --check'

    Write-Host ''
    Write-Host 'Atualização 62 V2 aplicada e validada com sucesso.' -ForegroundColor Green
    Write-Host "Backup local: $backup" -ForegroundColor Green
    Write-Host 'Nenhuma alteração foi feita no banco durante esta etapa local.' -ForegroundColor Green
    Write-Host ''
    git status --short
}
catch {
    ...
    if (Test-Path -LiteralPath $backupBuild) {
        Remove-Item -LiteralPath (Join-Path $Repo 'frontend\build') -Recurse -Force -ErrorAction SilentlyContinue
        New-Item -ItemType Directory -Path (Join-Path $Repo 'frontend') -Force | Out-Null
        Copy-Item -LiteralPath $backupBuild -Destination (Join-Path $Repo 'frontend\build') -Recurse -Force
    }
