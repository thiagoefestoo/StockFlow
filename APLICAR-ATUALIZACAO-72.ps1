...
param(
    ...
    [string]$Zip = "$HOME\Downloads\atualizacao-72-multiplos-documentos-perda.zip"
)
...
if ($existingChanges.Count -gt 0) {
    Write-Host "Existem alterações locais nos arquivos usados pela atualização 72:" -ForegroundColor Yellow
...
$temp = Join-Path $env:TEMP "estoque-superinfra-atualizacao-72-$stamp"
$backup = Join-Path $HOME "Documents\backup-estoque-superinfra-atualizacao-72-$stamp"
...
try {
    Write-Host "Extraindo atualização 72..." -ForegroundColor Cyan
    ...
    Write-Host "Testando múltiplos documentos por perda..." -ForegroundColor Cyan

    Invoke-NativeChecked `
        -Command "node" `
        -Arguments @("backend/scripts/test-loss-multiple-attachments.js") `
        -Description "Teste de múltiplos documentos"
    ...
    Write-Host ""
    Write-Host "Atualização 72 aplicada e validada com sucesso." -ForegroundColor Green
    Write-Host "Backup local: $backup" -ForegroundColor Green
    Write-Host "Nenhuma migration, saldo ou documento antigo foi alterado pela instalação." -ForegroundColor Green
    Write-Host ""
    git status --short
}
catch {
