...
param(
    ...
    [string]$Zip = "$HOME\Downloads\atualizacao-71-editar-quantidades-entrada-permissao.zip"
)
...
if ($existingChanges.Count -gt 0) {
    Write-Host "Existem alterações locais nos arquivos usados pela atualização 71:" -ForegroundColor Yellow
...
$temp = Join-Path $env:TEMP "estoque-superinfra-atualizacao-71-$stamp"
$backup = Join-Path $HOME "Documents\backup-estoque-superinfra-atualizacao-71-$stamp"
...
try {
    Write-Host "Extraindo atualização 71..." -ForegroundColor Cyan
    ...
    Write-Host "Testando a permissão e as proteções..." -ForegroundColor Cyan

    Invoke-NativeChecked `
        -Command "node" `
        -Arguments @("backend/scripts/test-stock-batch-quantity-edit-permission.js") `
        -Description "Teste da edição de quantidades"
    ...
    Write-Host ""
    Write-Host "Atualização 71 aplicada e validada com sucesso." -ForegroundColor Green
