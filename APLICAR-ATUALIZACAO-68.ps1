...
param(
    ...
    [string]$Zip = "$HOME\Downloads\atualizacao-68-busca-multiplos-seriais-retorno.zip"
)
...
if ($existingChanges.Count -gt 0) {
    Write-Host "Existem alterações locais na página Retorno Técnico:" -ForegroundColor Yellow
    $existingChanges | ForEach-Object { Write-Host $_ -ForegroundColor Yellow }
    throw "Faça commit ou backup dessas alterações antes de aplicar a atualização 68."
}

$stamp = Get-Date -Format 'yyyyMMdd-HHmmss'
$temp = Join-Path $env:TEMP "estoque-superinfra-atualizacao-68-$stamp"
$backup = Join-Path $HOME "Documents\backup-estoque-superinfra-atualizacao-68-$stamp"
...
try {
    Write-Host "Extraindo atualização 68..." -ForegroundColor Cyan
    ...
    Write-Host ""
    Write-Host "Atualização 68 aplicada e validada com sucesso." -ForegroundColor Green
    Write-Host "Backup local: $backup" -ForegroundColor Green
    Write-Host "Backend e banco de dados não foram alterados." -ForegroundColor Green
    Write-Host ""
    git status --short
}
catch {
