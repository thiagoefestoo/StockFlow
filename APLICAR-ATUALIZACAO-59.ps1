...
param(
    ...
    [string]$Zip = "$HOME\Downloads\atualizacao-59-segregacao-cidade-estoque-filtros.zip"
)
...
$temp = Join-Path $env:TEMP "estoque-superinfra-atualizacao-59-$stamp"
$backup = Join-Path $HOME "Documents\backup-estoque-superinfra-atualizacao-59-$stamp"
...
try {
    ...
    Write-Host "Validando diferenças do Git somente nos arquivos da atualização..." -ForegroundColor Cyan
    $gitCheckPaths = @(
        $patchFiles |
            ForEach-Object {
    ...
    Write-Host ''
    Write-Host 'Atualização aplicada e validada com sucesso.' -ForegroundColor Green
    ...
    $backupFiles = Get-ChildItem -LiteralPath $backup -File -Recurse | Where-Object {
