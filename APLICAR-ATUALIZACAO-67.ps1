...
param(
    ...
    [string]$Zip = "$HOME\Downloads\atualizacao-67-excluir-item-solicitacao.zip"
)
...
if ($existingChanges.Count -gt 0) {
    Write-Host "Existem alterações locais nos arquivos usados pela atualização 67:" -ForegroundColor Yellow
...
$temp = Join-Path $env:TEMP "estoque-superinfra-atualizacao-67-$stamp"
$backup = Join-Path $HOME "Documents\backup-estoque-superinfra-atualizacao-67-$stamp"
...
try {
    Write-Host "Extraindo atualização 67..." -ForegroundColor Cyan
    ...
    Write-Host "Testando exclusão parcial de itens..." -ForegroundColor Cyan

    Invoke-NativeChecked `
        -Command "node" `
        -Arguments @("backend/scripts/test-request-delivery-exclusions.js") `
        -Description "Teste da exclusão de itens"
    ...
    Write-Host ""
    Write-Host "Atualização 67 aplicada e validada com sucesso." -ForegroundColor Green
    Write-Host "Backup local: $backup" -ForegroundColor Green
    Write-Host "Banco de dados: nenhuma migration ou alteração automática." -ForegroundColor Green
    Write-Host ""
    git status --short
}
catch {
