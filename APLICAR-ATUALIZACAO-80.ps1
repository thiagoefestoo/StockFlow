...
param(
    ...
    [string]$Zip = "$HOME\Downloads\atualizacao-80-exibe-todos-itens-transferencia-estoques.zip"
)
...
if ($existingChanges.Count -gt 0) {
    Write-Host "Existem alterações locais nos arquivos da Atualização 80:" -ForegroundColor Yellow
...
$temp = Join-Path $env:TEMP "estoque-superinfra-atualizacao-80-$stamp"
$backup = Join-Path $HOME "Documents\backup-estoque-superinfra-atualizacao-80-$stamp"
...
try {
    Write-Host "Extraindo atualização 80..." -ForegroundColor Cyan
    ...
    Write-Host "Testando a listagem da transferência..." -ForegroundColor Cyan

    Invoke-NativeChecked `
        -Command "node" `
        -Arguments @("backend/scripts/test-warehouse-transfer-all-stock-items.js") `
        -Description "Teste da listagem de materiais"
    ...
    Write-Host ""
    Write-Host "Atualização 80 aplicada e validada com sucesso." -ForegroundColor Green
